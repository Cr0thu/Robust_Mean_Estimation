...
class Top_K_SGD(object):

    def __init__(self, params):
        self.params = params

    def SGD(self, S, iter_num):
        """Stage 1 algorithm."""
        d = self.params.d
        m = self.params.m
        eps = self.params.eps
        ...
        for t in range(max_iter):
            ...
            for i in range(K):
                idx_tmp = idx_split[i]
                S_tmp = [S[j] for j in idx_tmp]
                S_grouped = list(np.mean(S_tmp, axis = 0))
                S_grouped = np.array(S_grouped)
            
                grad_u = - \
                    np.sign(S_grouped.reshape(d) - u * u + v * v) * u
                grad_v = np.sign(S_grouped.reshape(d) - u * u + v * v) * v
                u -= eta * grad_u / K
                v -= eta * grad_v / K
            eta *= rho
    ...
    def alg(self, S, indicator):
        ...
        # print("GD: ", self.GD(S))
        #k = self.params.k
        estimated_mean, pred_k = Top_K.GD(self, S, 200)
        start_time = time.time()
        print("useless")
        estimated_mean, _ = self.SGD(S, 600)
        total_time = time.time() - start_time
